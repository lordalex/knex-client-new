...
app.post("/generatePINandticket", async (req, res) => {
    const context = await validateRequest(req, res);
    if (!context)
        return;
    try {
        ...
        const ticketData = Object.assign({ id: ticketId, userId: context.uid, pin: pin, status: "Arrival", createdAt: admin.firestore.FieldValue.serverTimestamp() }, context.data);
...
app.post("/createTicket", async (req, res) => {
    ...
    try {
        const ticketId = admin.firestore().collection("tickets").doc().id;
        const ticketData = Object.assign({ id: ticketId, userId: context.uid, status: "Parked", createdAt: admin.firestore.FieldValue.serverTimestamp() }, context.data);
        await admin.firestore().collection("tickets").doc(ticketId).set(ticketData);
...
// Get Latest Ticket
app.post("/getLatestTicket", async (req, res) => {
    const context = await validateRequest(req, res);
    if (!context)
        return;
    try {
...
app.post("/setTicketToCancel", async (req, res) => {
    const context = await validateRequest(req, res);
    if (!context)
        return;
    try {
...
// Set to Departure
app.post("/setToDeparture", async (req, res) => {
    const context = await validateRequest(req, res);
    if (!context)
        return;
    try {
...
// Set Ticket Tip
app.post("/setTicketTip", async (req, res) => {
    const context = await validateRequest(req, res);
    if (!context)
        return;
    try {
